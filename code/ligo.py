...
# Define Likelihoods to be sampled:
# Ligo Correlated Distribution to be sampled logP_0
def logP_0(x):
...
m = 30
test = ms.sph_tester(logP_0, m, 10*N)
...
samples, weights, mean_samples, walk_identity, tot_samp = run_mh_ref(logP_0, PT, x_0, N)
...
x_0 = np.random.uniform(size=(S,D)) * (2*pi,pi)
samples4, weights4, mean_samples4, walk_identity4, tot_samp4 = emcee(logP_0, Y, x_0, N, a)
...
x_0 = np.random.uniform(size=(S,D)) * (2*pi,pi)
samples1, weights1, mean_samples1, walk_identity1, tot_samp1 = run_mh_ref(logP_0, EA, x_0, N)
...
x_0 = np.random.uniform(size=(S,D)) * (2*pi,pi)
samples2, weights2, mean_samples2, walk_identity2, tot_samp2 = emcee(logP_0, X, x_0, N, a)
...
samples3, weights3, mean_samples3, walk_identity3, tot_samp3 = run_mh_ref(logP_0, U, x_0, N)
...
frac = 0.15     # Choose fraction of data points to ignore
b_ind = int(len(samples)*frac)
b_ind_eucem = int(len(samples4)*frac)

names = ["\phi","\\theta"]
labels =  ["\phi","\\theta"]
sph_samples = MCSamples(samples=samples[b_ind:], names=names, labels=labels)
...
emceeeuc_samples = MCSamples(samples=samples4[b_ind_eucem:], names=names, labels=labels)
